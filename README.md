...
- **防守決策** — 對手立直 / 默聽嫌疑 / 一向聽嫌疑時，分析該押牌還是 betaori
- **進攻決策** — 場上無威脅時，以切牌效率評分(切後向聽、有效進張、打點期望)
- **立直時機** — 門清聽牌時比較立直與默聽(役、打點、待牌品質、振聽、場況)
- **鳴牌判斷** — 量化「鳴線 EV vs 過線 EV」，復盤實際吃碰並指出漏鳴機會

hero 立直後的強制摸切不再計入決策。每個切牌決策輸出 chess.com 風格的五級評分：
...
### 打點估算 (hand_value.py) — 全役種覆蓋

把確定 yaku 與機率 yaku 都納入考量：

- **1-2 翻**:立直 / 役牌 (連風 2 翻) / 斷么 / 七對子路線 / 對對和路線 / 一通 /
  三色同順 / 三色同刻 / 三暗刻 / 小三元 / 混老頭 / 混全帶么九 / 一盃口
- **3+ 翻**:二盃口 / 純全帶么九 / 混一色 (+3/+2) / 清一色 (+6/+5)
- **役滿線** (直接 13 翻,不與一般役疊加):大三元、四暗刻、國士無雙、字一色、
  清老頭、綠一色、小四喜/大四喜、九蓮寶燈
- **分數翻**:自摸期望 (~25%)、裏寶期望 (立直線 +0.5)、平和可能 (+0.3)、役牌候補對子
- 副露資訊可用時,副露含么九會取消斷么判定;最後向上取整給整數翻數
- 模型外 (和牌瞬間運氣,無法從手牌預測):一發、海底/河底、嶺上、搶槓、天/地和、W立直、四槓子

### 自家振聽

聽牌但待牌在自己河中時,和率砍到 35% 並在理由中標註「振聽 (只能自摸)」。
...
- 跨多局統計儀表板
- 役種判定由 screen 式升級為完整面子分解 (消除 presence-based 的輕微高估)
